...
namespace School_MVVM.Common.ViewModel
{
    /// <summary>
    ...
    public class SingleObjectViewModelState<TPrimaryKey>
    {
    ...
    /// <typeparam name="TUnitOfWork">A unit of work type.</typeparam>
    [POCOViewModel]
    public abstract class SingleObjectViewModelBase<TEntity, TPrimaryKey, TUnitOfWork> : ISingleObjectViewModel<TEntity, TPrimaryKey>, ISupportParameter, IDocumentContent, ISupportLogicalLayout<SingleObjectViewModelState<TPrimaryKey>>
        where TEntity : class
        where TUnitOfWork : IUnitOfWork
    {
        ...
        {
            if (MessageBoxService.ShowMessage(string.Format(CommonResources.Confirmation_Save, typeof(TEntity).Name), "Saving", MessageButton.YesNo) != MessageResult.Yes)
                return;
            if (SaveCore())
                Close();
        }
        ...
        SingleObjectViewModelState<TPrimaryKey> ISupportLogicalLayout<SingleObjectViewModelState<TPrimaryKey>>.SaveState()
        {
            return new SingleObjectViewModelState<TPrimaryKey>
            {
                Key = PrimaryKey,
        ...
        void ISupportLogicalLayout<SingleObjectViewModelState<TPrimaryKey>>.RestoreState(SingleObjectViewModelState<TPrimaryKey> state)
        {
            LoadEntityByKey(state.Key);
            if (Entity == null)
